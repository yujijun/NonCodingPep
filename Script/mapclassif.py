...
def annotation(capaDf, headerFull, cDicSPE, cPersoTranscr, dbFm, dbFmIndex, cPP, nPP, cT, totcT, nT, totnT, out) :
    pattern = re.compile('ENSMUST[0-9]+')
                    ...
                    for a in accId.split('|') :
                        ...
                        if pcr is not None :
                            for p in pcr :
                                ...
                                line = list(row) + [dbOrigin, a, p] + agCode + [cFreq, nFreq] + [str(agBin)] + [ppDic.get(str(agBin), 'NA')]
                    ...
                    for a in accId.split('|') :
                        ...
                        if pcr is not None :
                            for p in pcr :
                                ...
                                line = list(row) + [dbOrigin, a, p] + agCode + [cFreq, nFreq] + [str(agBin)] + [speDic.get(str(agBin), 'NA')]
                                strLine = [str(el) for el in line]
                                o.write('\t'.join(strLine) + '\n')
                        else :
                            print 'No pcr associated to %s...' %(pep)
...
def main():
    ...
    headerFull = header + ['dbOrigin', 'protAccession', 'PCR', 'cancerProteomeCount', 'normalProteomeCount', 'cancerPCRCount', 'normalPCRcount', 'cancerPCRFreq', 'normalPCRFreq', 'agBinaryCode', 'immStatus']
    ...
    annotation(capaDf, headerFull, cDicSPE, cPersoTranscr, dbFm, dbFmIndex, cPP, nPP, cT, totcT, nT, totnT, out)
    print ''
